...
object TermuxInstallerK {
    //FIXEM  this kt file is auto-conversion from  Java to kotlin
    private const val TAG = "TermuxInstallerK"
            ...
            override fun run() {
                try {
                    ...
                    ZipInputStream(zipUrl.openStream()).use { zipInput ->
                        //FIXME Here will get error
                        var zipEntry: ZipEntry
                        while (zipInput.nextEntry.also {
                                zipEntry = it
                                //FIXME the error reason is  zipEntrynot use ? so if it is null can not assign to zipEntry
                            } != null) {
                            if (zipEntry.name == "SYMLINKS.txt") {
                                val symlinksReader = BufferedReader(InputStreamReader(zipInput))
                                var line: String
                                while (symlinksReader.readLine().also { line = it } != null) { //FIXME the SYMLINKS include “blank” so will error
